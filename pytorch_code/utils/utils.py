...
class CriteoDataset(Dataset):
    ...
    def __init__(self, file, read_part=True, sample_num=100000):
        ...
        # 原论文连续特征直接作为 DNN的输入，不参与 FM的输入。
        # 我们也可以连续变量离散化后，同时作为 DNN和 FM的输入，这样连续变量也和类别变量有二阶特征交互，或许可以带来更多的信息，增强模型表达。
        # 连续型特征等间隔分箱
        ...
        '''
        OrdinalEncoder是一个类（class），功能是将分类特征编码为整数数组。
        它的输入应该是整数或字符串的类数组，也可以理解为矩阵，每一列表示一个特征，每一个特征中的数字或者是字符串表示一类特征，
        也就是分类(离散)特征所接受的值。特征按顺序转换为有序整数。结果就是每个特征对应一个整数列，取值范围0到n_categories - 1。
        '''
...
class AmazonBooksDataset(Dataset):
    ...
    def __init__(self, file, read_part=True, sample_num=100000, sequence_length=40):
        ...
        cate_df = data_df['hist_cate_list'].apply(deal).join(data_df[['cateID']]).apply(cate_encoder.transform).join(
            data_df['label'])
        self.data = cate_df.values  # [hist_cate_list, cateID, label] [batch, hist_len+2]
...
class EarlyStopper:
    ...
    def __init__(self, model, num_trials=10):
        self.num_trials = num_trials  # How long to wait after last time validation loss improved.
