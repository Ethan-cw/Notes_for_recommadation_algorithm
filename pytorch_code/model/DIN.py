...
class ActivationUnit(nn.Module):
    ...
    def forward(self, x):
        behaviors = x[:, :-1]  # (batch, hist_len, embed_dim)
        query = x[:, -1:]
        _, num_behaviors, embed_dim = behaviors.shape
        queries = query.expand(-1, num_behaviors, -1)

        attention_input = torch.cat([queries, behaviors, queries - behaviors, queries * behaviors],dim=-1)
        att = self.mlp(attention_input)
