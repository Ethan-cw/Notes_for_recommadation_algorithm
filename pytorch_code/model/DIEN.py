...
def auxiliary_sample(data):
    ...
    """
    data = data.numpy()
    cate_max = np.max(data)  # 获取所有行为的code
    pos_sample = data[:, 1:-1]  #  去掉最后target cateID和无下一时刻的第一列
...
class EmbeddingLayer(nn.Module):
    ...
    def forward(self, x, neg_x=None):
        """
            input : 
                x : (behaviors * hist_len, ads * 1) -> [batch, (hist_len + ads)]
                neg_x : (behavior * (hist_len-1) ) -> [batch, (hist_len-1)]
            output :
                query_ad : (batch * 1 * embed_dim) (B * 1 * E)
                user_behavior : (batch * hist_len * embed_dim) (B * T * E)
                behavior_length : (B)
        """
        # 得到embedding表示, 以及pack_paded_sequence的输入
        behaviors_x = x[:, :-1]
        mask = (behaviors_x > 0).float().unsqueeze(-1)  #  (batch, hist_len, 1)
        ads_x = x[:, -1]
        ...
        behavior_length = (x[:, :-1] > 0).sum(dim=1).cpu()   # [batch]
...
class InterestExtractLayer(nn.Module):
    ...
    def forward(self, user_behavior, behavior_length, neg_user_behavior=None):
        ...
        pad_interests, _ = pad_packed_sequence(packed_interests, batch_first=True,
                                            padding_value=0.0, total_length=max_seq_len)
        
        if neg_user_behavior is not None:
            # 计算辅助loss
            gru_embed = pad_interests[:, :-1]  # 去掉最后一个时刻
            ...
            pos_res = pos_res.view(batch_size, aux_seq_len)[mask > 0].view(-1,1)  # 把非填充的保留下来
...
class AUGRUCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, bias=True):
        ...
        # W_ir | W_iz | W_ih
        self.weight_ih = nn.Parameter(torch.Tensor(3*hidden_size, input_size))  # (3*hidden_size, input_size)
        # W_hr | W_hz | W_hh
        self.weight_hh = nn.Parameter(torch.Tensor(3*hidden_size, hidden_size))
        self.register_parameter('weight_ih', self.weight_ih)
        self.register_parameter('weight_hh', self.weight_hh)
    ...
    def forward(self, inputs, hidden_state, att_score):
        # 计算 Wx + b
        gi = F.linear(inputs, self.weight_ih, self.bias_ih)  #  input * weight_ih + bias_ih
        gh = F.linear(hidden_state, self.weight_hh, self.bias_hh)
        ...
        # i_z : W_iz * x + b_iz
        # i_h : W_ih * x + b_ih
        i_r, i_z, i_h = gi.chunk(3, 1) # 沿 1轴分为3 块
        ...
        new_hidden = (1 - update_gate) * hidden_state + update_gate * new_hidden  # h
...
class DynamicGRU(nn.Module):
    ...
    def forward(self, inputs, att_score=None, hidden_state=None):
        ...
        if hidden_state is None:  # (batch, hidden_size)
...
class InterestEvolutionLayer(nn.Module):
    ...
    def forward(self, query_ad, pad_interests, behavior_length):
        ...
        # 学习 pack_padded_sequence https://zhuanlan.zhihu.com/p/342685890
        if self.gru_type == 'GRU':
            # GRU 接 Attention
            ...
            # pick last state
            out = InterestEvolutionLayer._get_last_state(out, behavior_length)  # [b, H]
        return out
...
class DeepInterestEvolutionNet(nn.Module):
    """
        深度兴趣进化网络
    """
    def __init__(self, feature_dim, embed_dim=8, hidden_size=8, mlp_dims=[64,32],
                 gru_type='AUGRU', dropout=0.2):
        super(DeepInterestEvolutionNet, self).__init__()
        self.feature_dim = feature_dim[0]
        self.embed_dim = embed_dim
        self.hidden_size = hidden_size
        self.gru_type = gru_type
        
        # Embedding layer
        self.embed_layer = EmbeddingLayer(self.feature_dim, embed_dim)
    ...
    def forward(self, x, neg_x=None):
        """
            x : (behaviors * hist_len, ads * 1) -> [batch, (hist_len+1)]
            neg_x : (behaviors * (hist_len-1)) -> [batch, (hist_len-1)]
        """
